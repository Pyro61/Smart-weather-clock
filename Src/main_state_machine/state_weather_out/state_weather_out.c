...
static void prepare_buf(struct weather_data data);
static void blink_signs(char *buf);
static void read_weather_data(void);


/* State functions */
static void weather_out_init(const struct display_interface *funs)
{
    ...
    /* Subscribe to data from external sensor arrival */
    if (events_subscribe(read_weather_data, EVENT_SEND) != SUBSCRIBE_SUCCESS) safe_state();
}


static void weather_out_on_entry(enum state_status last_state)
{
...
static void weather_out_on_exit(void)
{
...
static enum state_status weather_out_on_ok_mode_button_pressed(void)
{
    return MODE_SELECTION;
}


static enum state_status weather_out_on_right_button_pressed(void)
{
    return STATE_UNCHANGED;
}


static enum state_status weather_out_on_left_button_pressed(void)
{
    return STATE_UNCHANGED;
}


static enum state_status weather_out_on_up_button_pressed(void)
{
    return STATE_UNCHANGED;
}


static enum state_status weather_out_on_down_button_pressed(void)
{
    return STATE_UNCHANGED;
}


static enum state_status weather_out_on_refresh(void)
{
    blink_signs(buf);
...
static enum state_status weather_out_on_alarm(void)
{
    return ALARM;
}


static const struct main_state_interface state_weather_out = 
{
    weather_out_init,
    weather_out_on_entry,
    weather_out_on_exit,
    weather_out_on_ok_mode_button_pressed,
    weather_out_on_right_button_pressed,
    weather_out_on_left_button_pressed,
    weather_out_on_up_button_pressed,
    weather_out_on_down_button_pressed,
    weather_out_on_refresh,
    weather_out_on_alarm,
};


const struct main_state_interface *main_state_weather_out_get(void)
{
    return &state_weather_out;
}
...
{   
    ...
    check_alarm(buf);
    write_time_to_buf(buf, time_get());
}
...
static void read_weather_data(void)
{
    /* Copy data from reception buffer to data holder */
    adapter_sys_comm_copy_data((uint8_t*)&data);
}